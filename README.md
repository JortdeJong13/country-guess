...
The reference countries are used both to train and evaluate models and by the ML Server. The model registry stores trained models and identifies the model used for predictions. Evaluation data is read from the drawing database through the Drawing Store API.

For local model evaluation, point `DRAWING_STORE_URL` at your Drawing Store API:

```bash
just DRAWING_STORE_URL=http://127.0.0.1:8080 evaluate-model
```
